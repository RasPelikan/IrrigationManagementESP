...
# Emits build_info.h with the build timestamp and current git commit so the
# firmware can report its provenance on the status page. Regenerated every build
# (and gitignored) — the running device always reflects the binary that was
# actually flashed, not whatever was checked in at sketch-write time.
write_build_info() {
  local build_time git_commit
  build_time="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
  if git_commit="$(git -C "$SCRIPT_DIR" rev-parse --short HEAD 2>/dev/null)"; then
  ...
  cat > "$SCRIPT_DIR/build_info.h" <<EOF
// Generated by build-firmware.sh — do not edit, not checked into git.
#pragma once
#define FIRMWARE_BUILD_TIME "$build_time"
#define FIRMWARE_GIT_COMMIT "$git_commit"
EOF
  log "Build info: $build_time / $git_commit"
}
